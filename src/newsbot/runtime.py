...
def production_configuration_errors(settings: Settings) -> list[str]:
    ...
    if not all(
        ...
        errors.append("Telegram Pyrofork credentials")
    if not settings.max_token or settings.max_chat_id is None:
        errors.append("MAX Pyromax credentials")
    return errors
...
async def make_publishers(settings: Settings) -> tuple[dict[Platform, Publisher], list[Any]]:
    ...
    if not all(
        ...
        missing.append("Telegram Pyrofork environment")
    if not settings.max_token or settings.max_chat_id is None:
        missing.append("MAX Pyromax environment")
    if missing:
    ...
    try:
        max_client = await MaxApi(token=settings.max_token, password=settings.max_password or None)
    except Exception:
        await telegram.stop()
        raise
    ...
    return {
        Platform.TELEGRAM: TelegramPyroforkPublisher(telegram, settings.telegram_chat_id),
